const chocolateFeast = (n, c, m) => {
    let startingBars = Math.floor(n/c)
    let totalBarsEaten = startingBars
    let currentWrappers = startingBars
    while (currentWrappers >= m){
        console.log("beginning of iteration totalbarseaten and currentwrappers", totalBarsEaten, currentWrappers)
        let newBars = Math.floor(currentWrappers/m)
        let turnedInWrappers = (newBars*m)
        console.log("turnedinwrappers", turnedInWrappers)
        console.log("newbars", newBars)
        let leftOverWrappers = currentWrappers%m
        console.log("leftoverwrappers before eating new bars", leftOverWrappers)
        totalBarsEaten += newBars
        console.log("currentWrappers before calc", currentWrappers)
        currentWrappers = leftOverWrappers + newBars
        console.log("currentWrappers after calc", currentWrappers)
    }
